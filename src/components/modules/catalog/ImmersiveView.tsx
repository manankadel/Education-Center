...
export const ImmersiveView = ({ products, focusedIndex, setFocusedIndex }: ImmersiveViewProps) => {
    ...
    if (!product) return <div className="w-full min-h-screen grid place-items-center text-white/50">Loading...</div>;

    return (
        <div className="relative w-full min-h-screen flex items-center justify-center px-4 py-32 md:py-12">
                ...
                <div className="relative w-full h-full">
                    <AnimatePresence initial={false} custom={direction} mode="wait">
                        <motion.div 
                            ...
                            className="w-full h-auto md:h-[75vh] md:max-h-[650px] rounded-2xl border border-white/10 bg-black/30 backdrop-blur-xl shadow-2xl flex flex-col md:grid md:grid-cols-2 md:grid-rows-1 cursor-grab active:cursor-grabbing overflow-hidden"
                        >
                            ...
                            <div className="w-full h-full flex flex-col p-6 md:p-10">
                                <div className="flex flex-col flex-grow min-h-0">
                                    <div>
                                        <h1 className="font-display text-3xl md:text-4xl font-bold text-white">{product.title}</h1>
                                        {selectedVariant && (
                                            <div className="inline-block mt-3"> 
                                                <p className="font-mono text-xl text-white/90 bg-white/5 px-3 py-1 rounded-md">
